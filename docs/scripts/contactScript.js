...
const contactMainData = [{type: 'div', class: 'contactImgContainer', apnd: 'contactContainer'},
                        ...
                        {type: 'div', class: ['contactMainButton', 'siteButton'], apnd: 'contactImgContainer', inHL: 'Send an Email', isBtn: true, hrf: 'mailto:markfitz815@gmail.com', onSelf: true}]
...
function createContactSection(arr){
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        if(arr[i].isBtn){
            opnPg(document.getElementsByClassName(arr[i].class[0])[0], arr[i].hrf, arr[i].onSelf);
        }
    }
}
